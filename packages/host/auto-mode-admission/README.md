...
The service exposes generated direct Remotes for reading the current browser-safe projection, changing mode, and enabling or disabling one current callable exact binding. The projection keeps the complete current callable set separate from the Recommended winners. Entering Custom copies the current Recommended set atomically, after which any callable exact binding may be added or removed. A Custom key that later becomes unavailable or disappears from the Evidence Pack remains stored, but cannot route. The external plugin remains the sole owner of evidence acquisition, binding compilation, Host route materialization, band policy, and candidate resolution; this package neither reads AA nor changes derived binding state.
