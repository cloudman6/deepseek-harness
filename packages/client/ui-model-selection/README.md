...
The maintainer fork also recognizes the optional `dshAutoMode` Session projection used by the external AA-informed Auto experiment. When present, Auto is the first root-menu row, and the trigger, checked state, effective provider/model/optional effort, Light/Standard/Deep task-handling level, AA or configured-Deep-fallback basis, reason, and `experimental-unadmitted` status come directly from that Host projection. These task-handling levels do not alias provider effort names. A preceding decision rolls each changed model, effort, or level value into the effective value over 1.2 seconds; Auto and every changed target use DSH business blue and breathe twice, while unchanged values stay static and reduced-motion users retain the final text without movement. Each persisted selection change adds one chat-timeline route fact at the event's durable position, including level and basis; schema-v1 `fast`/`standard`/`strong` events remain readable only for existing Session replay, schema v2 remains the non-admission contract, and schema v3 adds exact route-admission facts without changing the displayed route shape. The client invokes `/auto [off]` through the command Remote, and a manual model or effort selection disables Auto before calling `session.selectModel`. Capability absence preserves the ordinary menu. This fork-pinned bridge is not part of a default composition or an official Auto compatibility contract; its rationale and verification live in the [experimental Auto model menu Agent Note](../../../.agents/notes/implemented/feature/2026-08-17-experimental-auto-model-menu.md).
