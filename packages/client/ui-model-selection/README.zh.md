...
维护者 fork 也识别外部 Phase 0P 实验使用的可选 `dshAutoMode` Session 投影。该投影存在时，Auto 是根菜单的第一行；触发器、勾选状态、有效模型／推理强度、层级、原因代码、解释和 `experimental-unadmitted` 标签都直接来自该 Host 投影。展开后的 Auto 详情以明确的“实际选择”标签列出有效模型和推理强度，避免与下方独立的手动模型／推理强度控件混淆。投影提供前一条决定时，每个发生变化的模型或 effort 值都会在 1.2 秒内从前一条决定滚动到有效选择；两者都变化时，两条轨道同时滚动。切换期间，Auto 和每个变化目标使用 DSH 业务蓝，随后以呼吸灯方式平滑亮灭两次，再恢复原有颜色。未变化的值保持静态；启用减少动态效果的用户保留最终值和切换文字而不播放动画。每次持久化的 `dsh-auto-mode/selection` 切换都会在聊天时间线的该事件持久位置增加一条通知。Phase 0P 插件在触发它的用户消息之后、实际请求之前追加该事件，因此通知位于产生结果的助手回复之前，显示前后模型与推理强度，以及层级、原因代码和解释；它是路由事实，不是助手消息。客户端经命令 Remote 调用 `/auto [off]`；手工选择模型或推理强度时，会先停用 Auto，再调用 `session.selectModel`。该能力缺席时，普通菜单保持不变。这个固定 fork 的桥不属于默认组合或官方 Auto 兼容约定；其理由和验证记录在[实验性 Auto 模型菜单 Agent Note](../../../.agents/notes/implemented/feature/2026-08-17-experimental-auto-model-menu.md)中。
