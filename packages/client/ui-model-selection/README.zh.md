...
维护者 fork 也识别外部 AA 驱动 Auto 实验使用的可选 `dshAutoMode` Session 投影。该投影存在时，Auto 是根菜单第一行；触发器、勾选状态、有效提供方／模型／可选 effort、轻量／常规／深度任务处理级别、AA 或已配置深度 fallback 依据、原因及 `experimental-unadmitted` 状态都直接来自 Host 投影。任务处理级别不映射厂商 effort 名称。投影提供前一条决定时，每个发生变化的模型、effort 或级别会在 1.2 秒内滚动至有效值；Auto 与每个变化目标使用 DSH 业务蓝并呼吸两次，未变化值保持静态，减少动态效果用户则保留最终文字但不播放位移。每条已持久化的选择变化都会在事件的持久位置增加一条包含级别和依据的聊天时间线路由事实；schema v1 的 `fast`／`standard`／`strong` 事件只为现有 Session 回放保留读取能力，schema v2 继续作为无 admission 契约，schema v3 增加精确 route-admission fact 但不改变展示 route 形状。客户端经命令 Remote 调用 `/auto [off]`；手工选择模型或推理强度时，会先停用 Auto，再调用 `session.selectModel`。该能力缺席时，普通菜单保持不变。这个固定 fork 的桥不属于默认组合或官方 Auto 兼容约定；其理由和验证记录在[实验性 Auto 模型菜单 Agent Note](../../../.agents/notes/implemented/feature/2026-08-17-experimental-auto-model-menu.md)中。
