...
The **Auto Mode** Web Settings section for route admission. It lazily reads the maintained Host bridge after the section mounts and shows Recommended and Custom as two explicit modes. Recommended enables the first route in each non-empty handling level under the external price, latency, and stable-identity ordering. The page also displays every other callable, unavailable, or excluded binding, Pack versus automatic-binding origin, AA capability/normalized-price/latency metrics, handling bands, and the Evidence Pack, snapshot, binding-registry, local-overlay, route-policy, and admission-policy identities used for the result. Each route card appends `(Default)` to its AA record label when the Host route omits the effort control, and the card and technical details show the explicit reasoning effort or `Default`; this label does not add an effort parameter to the request. Exclusions are grouped by readable model display name and show provider/model, optional effort, localized reason, and exact route IDs for technical inspection.
