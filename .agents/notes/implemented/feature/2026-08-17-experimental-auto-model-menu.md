...
The maintainer fork extends the existing `conversation.input.model` entry with an optional `dshAutoMode` Session projection. When the projection exists, the root menu places Auto before Model and Effort, marks the active Auto row with the standard check icon, and renders the latest tier, reason code, explanation, and `experimental-unadmitted` status. The compact trigger prefixes the projected model and reasoning effort with Auto.
...
Component tests pin menu order, checked state, projected provider/model/effort, missing-catalog display, live projection replacement, route explanation, and the disable-Auto-before-manual-selection order. Browser-plugin tests pin `/auto` and `/auto off` command carriage, `/model` mutual exclusion, and error mapping. A committed keyless assembled-Web snapshot pins Auto as the first checked row, the projected route absent from the advisory catalog, its `strong` reason, and the unadmitted label. A real Web composition with the external plugin verifies that a running simple task changes the visible selection from Flash/High to Auto/Flash/Off before completion, shows its `fast` reason, and returns to the ordinary trigger after a manual Pro selection.
