...
Required out-of-tree events carry runtime registration metadata. Before constructing a cold `Session`, the coordinator asks `SessionStore` for the exact registered namespace/version and payload schema; absence or incompatibility raises `SessionFormatUnsupportedError` without rewriting the artifact. A failed cold read is retryable after the owning plugin registers. Built-in catalog events and explicitly ignorable unknown events keep their existing paths. See [plugin-contributed session events](session.md).
